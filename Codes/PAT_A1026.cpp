// My code is modified on the Algorithm Notebook version.
// The notebook version has a bug that when there are several tables available, 
// it always chooses the one with earliest release time rather than the available one with smallest number.
// I fixed this bug by simply adding one for-loop after idx and i are determined.
#include <algorithm>
#include <cmath>
#include <cstdio>
#include <vector>
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (scanf("%d:%d:%d %d %d", &h, &m, &s, &trainTime, &isVIP))
            ;
    ...
    while (i < (int)player.size()) {
        ...
        // This for-loop fixes the bug.
        for (int j = 1; j <= k; j++) {
            if (table[j].endTime <= player[i].arriveTime) {
                idx = j;
                break;
            }
        }
    ...
    for (i = 0; i < (int)player.size() && player[i].startTime < edTime; i++) {
